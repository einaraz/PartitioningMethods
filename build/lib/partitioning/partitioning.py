...
class Partitioning(object):
    ...
    def _densityCorrections(self, method):
        ...
        # Aditional variables --------------------------------------------------
        mu = (
            Constants.MWdryair / Constants.MWvapor
    ...
    def _fillGaps(self, maxGaps):
        ...
        if maxGaps > 20:
            raise TypeError(
                "Too many consecutive points to be interpolated. Consider a smaller gaps."
            )
    ...
    def WaterUseEfficiency(self, ppath="C3"):
        ...
        cov_wq = (
            matrixCov["w_p"]["h2o_p"] * 10**-3
        )  # covariance of w and q (kg/m^2/s);
        cov_wc = (
            matrixCov["w_p"]["co2_p"] * 10**-6
        )  # covariance of w and c (kg/m^2/s);
        ...
        ci_mod_const_ppm = ci_const_ppm(
            P, leaf_T, Constants.Rco2, Constants.wue_constants[ppath]["const_ppm"]
        )
        ...
        ci_mod_linear = cica_linear(
            ...
            Constants.wue_constants[ppath]["linear"][1],
        )
        ci_mod_sqrt = cica_sqrt(
            ambient_co2, vpd, Constants.wue_constants[ppath]["sqrt"]
        )
        ...
        if ppath == "C3":
            ...
            if vpdm < 0 or m < 0:
                self.wue[
                    "opt"
                ] = np.nan  # In case of negative vapor pressure deficit or m
            else:
                self.wue["opt"] = (
    ...
    def partCEC(self, H=0.0):
        ...
        T_condition_ET = (
            (10**-3) * Constants.Lv * np.sum(auxT["h2o_p"] * auxT["w_p"]) / N
    ...
    def partREA(self, H=0):
        ...
        ET = (
            np.cov(wseries, qseries)[0][1] * (10**-3) * Constants.Lv
    ...
    def partFVS(self, W):
        ...
        if rho < 0:
            if A <= B < C:
                ...
                self.fluxesFVS = {
                    "ET": Fq * (10**-3) * Constants.Lv,
                    "Fc": Fc * 10**3,
                    "E": np.nan,
                    "T": np.nan,
                    "P": np.nan,
                    "R": np.nan,
                    "status": "13a not satisfied",
                }
