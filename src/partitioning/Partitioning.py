...
class Constants:
    Rd = 287 * ureg.joule / (ureg.kelvin * ureg.kilogram)  # Gas constant of dry air - J/K/kg
    ...
    MWdryair = 0.0289645 * ureg.kilogram / ureg.mole  # Molecular weight of dry air - kg/mol
    MWvapor = 0.018016 * ureg.kilogram / ureg.mole  # Molecular weight of vapor - kg/mol
    MWco2 = 0.044010 * ureg.kilogram / ureg.mole  # Molecular weight of carbon dioxide - kg/mol
    Rco2 = 8.3144598 * ureg.joule / (ureg.mole * ureg.kelvin) / MWco2  # Gas constant for CO2 - J/(mol K)/kg/mol
    Rvapor = 8.3144598 * ureg.joule / (ureg.mole * ureg.kelvin) / MWvapor  # Gas constant for water vapor - J/(mol K)/kg/mol
    diff_ratio = 1.6  # Ratio of diffusivities water/CO2 (dimensionless)
    g = 9.8160 * ureg.meter / ureg.second**2  # Gravity - m/s^2
    cp = 1.005 * ureg.kilojoule / (ureg.kilogram * ureg.kelvin)                   # specific heat capacity at constant pressure (kJ/kg/K)                       
    wue_constants = {
        "C3": {
...
class Partitioning(object):
    ...
    def __init__(self, hi, zi, freq, length, df, PreProcessing, argsQC):
        ...
        if PreProcessing:
            ...
            if self.argsQC.get("time_lag_correction"):
                self._time_lag_correction(max_lag_seconds=self.argsQC.get("max_lag_seconds"), type_lag=self.argsQC.get("type_lag"), saveplotlag=self.argsQC.get("saveplotlag") )
            self._fluctuations(method=self.argsQC.get("fluctuations"), filter_cut=self.argsQC.get("filtercut"))
    ...
    def _time_lag_correction(self, max_lag_seconds, type_lag, saveplotlag):
        ...
        """        
        lag_co2, lag_h2o = max_time_lag_crosscorrel(df=self.data[['co2', 'h2o', 'w']], sampling_freq=self.freq.magnitude, max_lag_seconds=max_lag_seconds, type_lag=type_lag, saveplotlag=saveplotlag )
    ...
    def _fluctuations(self, method, filter_cut):
        ...
        if method == "LD":
            ...
            for ii, _var in enumerate(Lvars):
                self.data[_var + "_p"] = FilterLowFrequencies(self.data[_var].values, self.freq.magnitude, filter_cut)
        elif method == "BA":
            for ii, _var in enumerate(Lvars):
    ...
    def _densityCorrections(self, method):
        ...
        # Aditional variables --------------------------------------------------
        mu = (Constants.MWdryair.magnitude / Constants.MWvapor.magnitude)  # ratio of dry air mass to water vapor mass
    ...
    def TurbulentStats(self):
        ...
        d         = (2./3.) * self.hi                                    # Displacement height [m]
        Qv        = matrixCov["Ts_p"]["w_p"] * self.units["Ts_p"] * self.units["w"]  # kinematic heat flux [K m/s]
        meanTkelvin   = (self.data["Tv"].mean() + 273.15)* self.units["Ts_p"]
        rho_moist_air = self.data["rho_moist_air"].mean() * self.units["rho_moist_air"] # kg/m3
        ...
        self.turbstats = { 'ustar': ustar * self.units["u"],                   
                           'cstar': cstar,      
                           'qstar': qstar,                
                           'tstar': tstar ,              
                            'zeta': zeta,  
    ...
    def WaterUseEfficiency(self, ppath="C3"):
        ...
        rho_totair = P / (Constants.Rd.magnitude * (mean_Tv + 273.15))  # moist air density (kg/m^3)
        ustar = (matrixCov["u_p"]["w_p"] ** 2.0 + matrixCov["w_p"]["v_p"] ** 2.0) ** (1.0 / 4.0)  # friction velocity [m/s]
        Qv = matrixCov["Tv_p"]["w_p"]  # kinematic virtual temperature flux [K m/s]
        dd = self.hi.magnitude * (2 / 3)  # displacement height [m]
        zeta = (-Constants.VON_KARMAN * Constants.g.magnitude * (self.zi.magnitude - dd) * Qv/ ((mean_Tv + 273.15) * (ustar**3)))  # Monin-Obukhov stability parameter
        ...
        arg = (np.log((self.zi.magnitude - dd) / zv) - psi_v) / Constants.VON_KARMAN / ustar
        ...
        ci_mod_const_ppm = ci_const_ppm(P, leaf_T, Constants.Rco2.magnitude, Constants.wue_constants[ppath]["const_ppm"].magnitude)
        ci_mod_const_ratio = cica_const_ratio(ambient_co2, Constants.wue_constants[ppath]["const_ratio"])
        ci_mod_linear = cica_linear(ambient_co2,vpd,Constants.wue_constants[ppath]["linear"][0],Constants.wue_constants[ppath]["linear"][1].magnitude,)
        ci_mod_sqrt = cica_sqrt(ambient_co2, vpd, Constants.wue_constants[ppath]["sqrt"].magnitude)
        ...
        if ppath == "C3":
            ...
            vpdm = vapor_press_deficit_mass(ambient_h2o, leaf_T, Constants.Rvapor.magnitude)
            if vpdm < 0 or m < 0:
    ...
    def partCEC(self, H=0.0):
        ...
        E_condition_ET = ((10**-3) * Constants.Lv.magnitude * np.sum(auxE["h2o_p"].values * auxE["w_p"].values)/ N)  # conditional flux [1st quadrant and w'>0] flux given in  W/m2
    ...
    def partREA(self, H=0):
        ...
        ET = (np.cov(wseries, qseries)[0][1] * (10**-3) * Constants.Lv.magnitude)  # latent heat flux [W/m2]
    ...
    def partFVS(self, W):
        ...
        if rho < 0:
            if A <= B < C:
                ...
                self.fluxesFVS = {
                    "ETfvs": Fq * (10**-3) * Constants.Lv.magnitude * ureg.watt/ureg.meter**2,
    ...
    def partCEA(self, H=0.00):
        ...
        total_ET = (auxETcov["h2o_p"]["w_p"] * unitLE)  # flux [all quadrants] given in  W/m2

        # Creates a dataframe with variables of interest and conditioned on updrafts and on the first quadrant
        auxE = df[(df["w_p"] > 0)&(df["co2_p"] > 0)&(df["h2o_p"] > 0)& (abs(df["co2_p"] / df["co2_p"].std())> abs(H * df["h2o_p"].std() / df["h2o_p"]))][["co2_p", "h2o_p", "w_p"]]
        auxE_n = df[(df["w_p"] < 0)& (df["co2_p"] < 0)& (df["h2o_p"] < 0)& (abs(df["co2_p"] / df["co2_p"].std())> abs(H * df["h2o_p"].std() / df["h2o_p"]))][["co2_p", "h2o_p", "w_p"]]
        C1 = auxE["co2_p"].mean()
        C2 = auxE_n["co2_p"].mean()
        Q1 = auxE["h2o_p"].mean()
        Q2 = auxE_n["h2o_p"].mean()
        sum1 = (auxE["co2_p"].index.size / N) * 100  # Number of points on the first quadrant
        sum2 = (auxE_n["co2_p"].index.size / N) * 100  # Number of points on the first quadrant

        # Creates a dataframe with variables of interest and conditioned on updrafts and on the second quadrant
        auxT = df[(df["w_p"] > 0)& (df["co2_p"] < 0)& (df["h2o_p"] > 0)& (abs(df["co2_p"] / df["co2_p"].std())> abs(H * df["h2o_p"].std() / df["h2o_p"]))][["co2_p", "h2o_p", "w_p"]]
        auxT_n = df[(df["w_p"] < 0)& (df["co2_p"] > 0)& (df["h2o_p"] < 0) & (abs(df["co2_p"] / df["co2_p"].std())> abs(H * df["h2o_p"].std() / df["h2o_p"]))][["co2_p", "h2o_p", "w_p"]]
        C3 = auxT["co2_p"].mean()
        C4 = auxT_n["co2_p"].mean()
        Q3 = auxT["h2o_p"].mean()
        Q4 = auxT_n["h2o_p"].mean()
        sum3 = (auxT["co2_p"].index.size / N) * 100  # Number of points on the first quadrant
        sum4 = (auxT_n["co2_p"].index.size / N) * 100  # Number of points on the first quadrant
    ...
    def partCECw(self, W, H=0.00):
        ...
        if W > (total_Fc / ( 10**3 * total_ET) ):
            self.fluxesCECw = {
                "ETcecw": total_ET * (10**-3) * Constants.Lv.magnitude * ureg.watt/ureg.meter**2,
        ...
        # Creates a dataframe with variables of interest and conditioned on updrafts and on the second quadrant
        auxT = df[(df["w_p"] > 0)&(df["co2_p"]<0)&(df["h2o_p"]>0)&(abs(df["co2_p"] / df["co2_p"].std())> abs(H * df["h2o_p"].std() / df["h2o_p"]))][["co2_p", "h2o_p", "w_p"]]
        P_condition_Fc = (sum(auxT["co2_p"] * auxT["w_p"]) / N)  # conditional flux [2nd quadrant and w'>0] given in mg/(s m2)
        T_condition_ET = (sum(auxT["h2o_p"] * auxT["w_p"]) / N)  # conditional flux [2nd quadrant and w'>0] flux given in  W/m2
        ...
        if T_condition_ET > 0:
            ...
            self.fluxesCECw = {
                "ETcecw": total_ET  * (10**-3) * Constants.Lv.magnitude * ureg.watt/ureg.meter**2,
                 "Ececw": total_ET  * (10**-3) * Constants.Lv.magnitude * ureg.watt/ureg.meter**2,
        ...
        if ratioET > 0:
            ...
            self.fluxesCECw = {
                "ETcecw": total_ET  * (10**-3) * Constants.Lv.magnitude * ureg.watt/ureg.meter**2,
                 ...
                 "Tcecw": total_ET * (10**-3) * Constants.Lv.magnitude * ureg.watt/ureg.meter**2,
                "Fccecw": total_Fc * ureg.milligram/ureg.meter**2/ureg.second,
